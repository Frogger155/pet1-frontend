...
export default function EditPost() {
    ...
    let post_tags_with_ids = []

    for (let tag of post_tags) {
        post_tags_with_ids.push({
            id: id++,
            name: tag["name"]
        })
    }

    console.log(post_tags_with_ids)
    const [tagsList, setTagsList] = useState(post_tags_with_ids)
            ...
            .then(
                response => {
                    ...
                    if (response.status < 400) {
                        setInfoText("Пост успешно отредактирован. Перенаправляем вас на другую страницу")
                    }
                }
            )
            .catch(
        ...
        <>
            <div class="flex flex-col items-center justify-center">
                <div class="w-1/2 border rounded-md shadow-slate-700 shadow-sm">
                    <h4 class="text-center m-1 text-black font-bold text-xl uppercase">Редактирование поста</h4>
                    <div class="p-10 rounded-lg">
                        <form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            <div class="mb-2">
                                <img src={"http://localhost:8000" + post["content"]} alt="Post content" style={{ width: '100%', maxHeight: '300px' }} />
